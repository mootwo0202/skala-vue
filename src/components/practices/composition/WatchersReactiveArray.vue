...
watch(cityList, (newArr, oldArr) => {logAuto.value = `[자동 감시] 배열 변동 발생! 옛날길이인척하는:${oldArr.length} / 현재길이:${newArr.length}`})
watch(() => [...cityList],
  (newArr, oldArr) => {logCopy.value = `[스냅샷 감시] 진짜 과거 길이:${oldArr.length} (데이터: ${oldArr}) ➡️ 바뀐 길이:${newArr.length}`})
</script>
