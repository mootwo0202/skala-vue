  ...
  <div class="dashboard-wrapper">
    ...
    <section class="list-box">
      ...
      <div
        ...
        class="weather-card"
        @click="
          selectedCityInfo = `${item.name}이 선택되었습니다. 예상 강수량은 ${item.rainfall}mm입니다.`
        "
      >
        <h4>{{ item.name }} ({{ item.status }})</h4>
        <p>현재 기온: {{ item.temp }}°C</p>
        <p v-if="item.rainfall > 0">예상 강수량: {{ item.rainfall }}mm ☔</p>
        <p v-else>예상 강수량: 0mm ☀️</p>

        <span v-if="item.temp >= 30" class="badge hot">🔥🔥 매우 더움 (30도 이상)</span>
        <span v-else-if="item.temp >= 25" class="badge hot">🔥 더움 (25도 이상)</span>
        <span v-else class="badge cool">❄️ 선선함 (25도 미만)</span>

        <button
          class="btn-detail"
          @click.stop="showDetail(item.name, item.status, item.rainfall)"
        >
          상세보기
        </button>
      </div>
